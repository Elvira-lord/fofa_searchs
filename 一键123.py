...
#存储
#将存储的列表进行存储
def save_result(list,last_row,excel_path,lock=None):
    '''
    #接下来 将得到的数据写入excel
    ...
    try:
        wb=load_workbook(excel_path)
        ws=wb.active
        while ws[f'A{last_row[0]}'].value is not None:
            last_row[0]+=1
        ws[f'A{last_row[0]}']=list[0]  #url
        ws[f'B{last_row[0]}']=list[1]  #ip
        ws[f'C{last_row[0]}']=list[2]  #端口
        ws[f'D{last_row[0]}']=list[3]  #状态码
        if len(list[4])==0:
            ws[f'E{last_row[0]}']='!!!未找到标题'
        else:
            ws[f'E{last_row[0]}']=list[4][0]  #标题
        ws[f'F{last_row[0]}']=list[5]  #端口类型
        ws[f'G{last_row[0]}']=list[6]  #服务
        wb.save(excel_path)
    finally:
        if lock:
            lock.release()
...
def save_other_status_code(list,status_code,save_code_last_row,excel_path,lock=None):
    if lock:
        lock.acquire()
    try:
        wb=load_workbook(excel_path)
...
#探活
#=======================================================================
def request_url(ips,ip_type,code_200,last_row,save_code_last_row,save_code,excel_path,lock=None):
    #调用前面的随机ua
    ...
    save_result(result,last_row,excel_path,lock)
    last_row[0] += 1  #每次加1
    if statusCode==200:
        code_200[0]+=1
    if statusCode in save_code:
        save_other_status_code(result,statusCode,save_code_last_row,excel_path,lock)
...
def ping(save_code,results_list,excel_path):
    ...
    #多线程并发配置
    lock = threading.Lock()  #线程锁，保护Excel写入
    ...
    def process_ip(ips):
        ...
        if ips[0].startswith('http'):
            pass
        request_url(ips,ip_type,code_200,last_row,save_code_last_row,save_code,excel_path,lock)

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(process_ip, results_list)
